    ...
    for (auto MI = MDlist.begin(), ME = MDlist.end(); MI != ME; ++MI) {
      if (MI->getName().equals("opencl.kernels")) {
        for (unsigned i = 0; i != MI->getNumOperands(); ++i) {
          ...
          if (dyn_cast<ValueAsMetadata>(op->getOperand(0).get())->getValue() ==
              &F) {
            kernelMD = op;
          }
        }
...
private:
  ...
  public:
    ...
    void visitAddrSpaceCastInst(AddrSpaceCastInst &I) {
      ...
      if (I.getSrcTy()->getPointerAddressSpace() == 1 &&
          I.getDestTy()->getPointerAddressSpace() == 0) {
        auto newPTI = new PtrToIntInst(
            I.getOperand(0), Type::getInt64Ty(I.getContext()), "", &I);
    ...
    void visitLoadInst(LoadInst &I) {
      if (GEPOperator *GEP = dyn_cast<GEPOperator>(I.getOperand(0))) {
        if (auto CASC = dyn_cast<ConstantExpr>(GEP->getPointerOperand())) {
          if (auto IASC =
                  dyn_cast<AddrSpaceCastInst>(CASC->getAsInstruction())) {
            SmallVector<Value *, 8> Indices(GEP->idx_begin(), GEP->idx_end());

            auto elemTy =
                IASC->getOperand(0)->getType()->getPointerElementType();
    ...
    void visitCallInst(CallInst &I) {
      for (unsigned int i = 0; i != I.getNumOperands(); ++i) {
        ...
        if (AddrSpaceCastInst *asc =
                dyn_cast<AddrSpaceCastInst>(I.getOperand(i))) {
      ...
      if (Operator *Cast = dyn_cast<Operator>(I->getOperand(Idx))) {
        ...
        if (eleminate) {
          ...
          if (SrcTy->getElementType() != DestTy->getElementType()) {
            BitCastInst *BitCast =
                new BitCastInst(Operand,
                                DestTy->getElementType()->getPointerTo(
  ...
  public:
    ...
    void visitInsertValueInst(InsertValueInst &I) {
      auto ExTy = ExtractValueInst::getIndexedType(
          I.getAggregateOperand()->getType(), I.getIndices());
    ...
    void visitICmpInst(ICmpInst &IC) {
      ...
      if (Op0Ty && Op1Ty && Op0Ty != Op1Ty) {
        ...
        //__dump(IC);
        auto PTI0 = PtrToIntInst::Create(Instruction::CastOps::PtrToInt,
                                         IC.getOperand(0),
                                         Type::getInt64Ty(IC.getContext()));
        auto PTI1 = PtrToIntInst::Create(Instruction::CastOps::PtrToInt,
                                         IC.getOperand(1),
                                         Type::getInt64Ty(IC.getContext()));
