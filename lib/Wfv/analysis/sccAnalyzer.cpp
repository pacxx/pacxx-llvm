                     ...
                     const SmallVector<SCCEdge*, 4>&        ignoreEdges)
{
    ...
    if(mInfo->mVerbose) outs() << "findSCC(" << node->mBlock->getName() << "(" << nextIndex << "))\n";
    ...
    for (unsigned i=0, e=terminator->getNumSuccessors(); i<e; ++i)
    {
        ...
        if (!succNodeOnStack)
        {
            if(mInfo->mVerbose) outs() << "  successor " << succBB->getName() << " is not in stack.\n";
            continue;
        }

        if(mInfo->mVerbose) {
            outs() << "  successor " << succBB->getName();
            outs() << " is in stack -> in current SCC.\n";
        }
                               ...
                               SmallVector<SCCEdge*, 4>& ignoreEdges)
{
    ...
    if(mInfo->mVerbose) {
        outs() << "ignored edge for next iteration: '" << entryBB->getName();
        outs() << "' -> '" << header->getName() << "\n";
    }
}
